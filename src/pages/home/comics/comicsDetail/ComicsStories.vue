  ...
  <section class="flex flex-center">
    <div class="row full-width">
      <div class="col-12 q-pa-sm">
        <q-list bordered>
          <q-expansion-item
            v-for="item in getComicsStories.results"
            :key="item.id"
            group="storiesGroup"
      ...
      <div class="q-pa-lg flex flex-center col-12" v-if="getComicsStories.total > perPage">
        <q-pagination
          size="15px"
          v-model="currentPage"
          :max="maxPagination"
...
export default {
  ...
  computed: {
    ...mapGetters('ComicsModule', ['getComicsStories']),

    maxPagination () {
      return Math.ceil(this.getComicsStories.total / this.perPage)
    }
  },

  methods: {
    ...mapActions('ComicsModule', ['fetchComicsStories']),

    fetchNextStories () {
      let offset = this.perPage * (this.currentPage - 1)
      this.fetchComicsStories({
